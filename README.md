...
- **SQLite** (используется по умолчанию в dev/тестах): один writer-процесс.
  Для прода доступен PostgreSQL-бэкенд через `DATABASE_URL`
  (`postgresql+asyncpg://...`) с пулом соединений и healthcheck; развернуть
