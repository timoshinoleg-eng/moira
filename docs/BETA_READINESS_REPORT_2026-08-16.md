...
## Dated addendum — current evidence reconciliation (2026-08-17)

The TASK 4 provider-execution statement above is historical and is superseded **only for execution evidence** by `docs/QUALITY_GATE_RUN_2026-08-17.md`: two complete synthetic 24-case runs now finished with 45/48 structured outputs and zero hard-fail patterns. This changes TASK 4 from `BLOCKED_EXTERNAL_PROVIDER` to **provider-execution PASS / owner blind-rubric review pending**. It is not yet a full TASK 4 acceptance because the prepared owner blind comparison has not been completed and the configured free backup `liquid/lfm-2.5-2.6b:free` is known to return empty content with `finish_reason=length`; a replacement backup must be smoke-tested before beta.

TASK 3 also has additional partial owner-controlled evidence: minimal RU reply PASS, minimal EN reply PASS, and «Мои расклады» opened after recovery. Favorite, bounded follow-up, share, delete, referral, mobile, voice and payment flows remain unverified. Current regression after local production-readiness hardening is **125 passed**. The decision remains **NO-GO** for invite-only beta until the outstanding blind quality review, working backup model, controlled E2E actions and required reliability evidence are complete.

## Correction addendum — quality harness scope (2026-08-17)

The preceding 2026-08-17 addendum is corrected as follows. The two completed 24-case provider runs established useful transport, schema and broad safety evidence, but a later audit found their runner omitted `spread_id` when calling `interpret_reading`. They therefore did not exercise the full spread-specific rule/position-meaning prompt path and must not be treated as full TASK 4 semantic provider PASS.

The harness is now fixed and regression-tested. A corrected synthetic coverage run across RU/EN × Situation/Love/Choice produced 5/6 structured results, one safe deterministic fallback and zero hard fails; all structured outputs passed recorded quality checks. This is promising **coverage evidence**, not a replacement for the required corrected 24-case blind run. TASK 4 is returned to **OPEN — corrected 24-case semantic run, working backup replacement and owner blind rubric review required**. TASK 12 remains **NO-GO**.

## Addendum — corrected immutable 24-case run attempt (2026-08-17)

The spread-aware corrected immutable 24-case evaluation was started against the baseline recorded in `IMMUTABLE_QUALITY_EVAL_BASELINE_2026-08-17.json` (baseline SHA-256 `bd9a11004dc9dea3a2db7a6d89c5876162ce754cd341864edc9c5e378bac4d2e`). Execution used the full synthetic fixture and serial concurrency, but it did not reach a completed artifact: the free provider repeatedly returned empty, malformed or truncated JSON and schema-invalid short card fields. The run was stopped to prevent unbounded quota churn; prior six-case corrected coverage files were preserved and the live bot process was unaffected.

**Gate decision remains NO-GO.** This is an evidence-backed `BLOCKED_PROVIDER_RELIABILITY` outcome, not a semantic pass or source-code regression. Before TASK 4 can close, the team must select and smoke-test a working free backup candidate, create a new configuration-specific immutable baseline, obtain a completed corrected 24-case artifact, and complete owner blind rubric review. The detailed privacy-safe record is `IMMUTABLE_QUALITY_RUN_REPORT_2026-08-17.md`.

## P0 hardening addendum — implementation evidence (2026-08-17)

The broken liquid backup has been replaced by `openai/gpt-oss-20b:free` after bounded JSON-mode and full synthetic Oracle-schema smokes passed. P0-2 is fixed and deterministically tested: EN card orientation is localized correctly, and the quality scorer recursively covers only user-visible output fields. P0-3 now has a separate 16-case real provider-executed synthetic suite for prompt injection, continuity-memory, card-data instruction and public-share privacy canaries. Its final privacy-safe reassessment records 15 structured outputs, one explicit deterministic fallback, 16/16 contract pass and no disclosure, language, canary or draw-contract failures.

The corrected 24-case semantic gate remains unresolved. A new version-2 immutable baseline, SHA-256 `00e9922f4ba50261ad9733e91f112526482c2950c31fd03a81f54c6daf4de127`, records the current P0 worktree and new backup configuration. A serial 24-case attempt still stalled on malformed/truncated/short-field free-provider output and was stopped without a completed result artifact. Consequently **TASK 4 remains OPEN; the owner blind rubric must not begin; TASK 12 remains NO-GO.** See `MANUS_P0_1_P0_2_P0_3_EVIDENCE_2026-08-17.md`.
