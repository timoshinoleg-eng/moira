...
from scripts.create_governing_eval_manifest import response_contract
...
def _fallback_draw_contract(payload: dict, drawn: list[DrawnCard], lang: str, spread_id: str) -> bool:
    ...
    for text, expected in zip(card_texts, drawn):
        ...
        if lang == "ru":
            expected_orientation = "перевёрнутая" if expected.reversed else "прямая"
        else:
            expected_orientation = "reversed" if expected.reversed else "upright"
...
def _write_json_atomic(path: Path, value: dict) -> None:
    ...
    if temporary.exists():
        raise ManifestValidationError(f"pending output already exists: {temporary.name}")
    temporary.write_text(json.dumps(value, ensure_ascii=False, indent=2), encoding="utf-8")
    temporary.replace(path)


async def run_manifest(manifest_path: Path, *, validate_only: bool = False) -> dict:
    """Validate and, unless requested otherwise, execute one bound Manifest-v2 run."""
    application_cfg = load_config(require_token=False)
    contract = response_contract(application_cfg)
    ...
    if bound.run_db_path.exists():
        raise ManifestValidationError("planned run database already exists")
    if validate_only:
        return {
            "run_id": bound.data["run_id"],
            "manifest": str(bound.path),
            "terminal_status": "PLANNED",
            "validation": "PASS",
            "provider_calls": 0,
        }

    cfg = replace(application_cfg, db_path=str(bound.run_db_path))
    cases = json.loads(bound.fixture_path.read_text(encoding="utf-8"))
    if [case["case_id"] for case in cases] != bound.data["fixture"]["case_ids"]:
        raise ManifestValidationError("fixture changed after manifest validation")

    public_results: list[dict] = []
    private_results: list[dict] = []
    fallback_counterparts: list[dict] = []
    ...
    try:
        ...
        for case in cases:
            ...
            fallback = compose_fallback_reading(
                case["lang"], case["spread"], delivery.drawn, case["question"]
            )
            fallback_delivery = EvaluationDelivery(
                "deterministic_fallback", fallback, delivery.drawn
            )
    ...
    modes = {
        mode: sum(item["status"] == mode for item in public_results)
    ...
    contract_fail_cases = [
        ...
        if item.get("checks") and not all(item["checks"].values())
    ]
    if hard_fail_cases or contract_fail_cases:
        terminal_status = "FAILED_CONTRACT"
    elif modes["failed_to_complete"]:
        terminal_status = "BLOCKED_PROVIDER_RELIABILITY"
    else:
    ...
    public = {
        ...
        "contract_fail_cases": contract_fail_cases,
    ...
    private = {
        "run_id": bound.data["run_id"],
        "prompt_version": PROMPT_VERSION,
        "items": private_results,
    }
    validate_output_documents(bound.data, public, private)
    _write_json_atomic(bound.public_output, public)
    _write_json_atomic(bound.private_output, private)
    verified_status = terminalize_manifest(
        bound.path,
        repo_root=ROOT,
        current_contract=contract,
    ...
    return {
        "run_id": bound.data["run_id"],
        "manifest": str(bound.path),
        "public_output": str(bound.public_output),
        "private_output": str(bound.private_output),
        "terminal_status": verified_status,
        "terminal_mode_counts": modes,
    }


def cli_main() -> None:
    parser = argparse.ArgumentParser(description="Execute one hash-bound governing Manifest-v2.")
    parser.add_argument("--manifest", required=True)
    parser.add_argument(
        "--validate-only",
        action="store_true",
        help="Validate every bound input without creating a DB, output, or provider call.",
    )
    args = parser.parse_args()
    try:
        result = asyncio.run(run_manifest(Path(args.manifest), validate_only=args.validate_only))
